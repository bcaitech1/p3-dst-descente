...
def make_turn_label(slot_meta, last_dialog_state, turn_dialog_state,
        ...
        try:
            ...
            if vv != v:
                if v == 'dontcare' and OP_SET[op_code].get('dontcare') is not None:
                    ...
                    generate_y.append([tokenizer.tokenize(v) + ['[EOS]'], idx]) # update해야하는 내용 tokenize
    ...
    for k, v in last_dialog_state.items():
        ...
        try:
            ...
            continue
    gold_state = [str(k) + '-' + str(v) for k, v in turn_dialog_state.items()] # 이번 turn의 정답
    if len(generate_y) > 0:
    ...
    if dynamic:
        ...
        generate_y = [tokenizer.convert_tokens_to_ids(y) for y in generate_y] # update해야하는 내용
...
class MultiWozDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        ### max_value error 방지 위해 바꿈 ### -> 다시 원상태로 복원. 
