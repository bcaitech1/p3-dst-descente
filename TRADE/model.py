...
class GRUEncoder(nn.Module):
    ...
    def forward(self, input_ids):
        # input_ids: [batch_size, max(len(input_id))], max(len(input_id))<=512
        # input_ids에서 pad_ids(=0)와 같으면 True
        mask = input_ids.eq(self.pad_idx).unsqueeze(-1)  # [batch_size, max(len(input_id)), 1]
        x = self.embed(input_ids)  # [batch_size, max(len(input_id)), embedding_size=768]
        if self.proj_layer:
            x = self.proj_layer(x)
        x = self.dropout(x)  # [batch_size, max(len(input_id)), embedding_size]
        o, h = self.gru(x)  # o: [batch_size, max(len(input_id)), num_directions * hidden_size], h: [num_layers * num_directions, batch_size, hidden_size]
        o = o.masked_fill(mask, 0.0)  # o: [batch_size, max(len(input_id)), num_directions * hidden_size]
        # bidirect은 forward와 backward 결과가 concat해서 나와서 decoder는 hidden size*2가 되어야함
        # 이를 hidden size로 통일하기 위해서 concat 결과를 sum으로 바꾼 코드
        # output: 전체 time step, hidden: forward, backward pass의 마지막을 sum
        # -> context vector로 사용, Decoder의 initial hidden state
        output = o[:, :, : self.d_model] + o[:, :, self.d_model :]  # [batch_size, max(len(input_id)), hidden_size], 전체 time step
        hidden = h[0] + h[1]  # [batch_size, hidden_size], pooled_output, n_layer 고려
...
class SlotGenerator(nn.Module):
    def __init__(
        ...
        # 이 부분은 GRUEncoder와 같은 코드인데 여기도 이게 왜 필요하지...
        if proj_dim:
