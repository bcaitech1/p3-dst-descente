...
def make_WOS_like_WOZ(file_path='./train_dials.json', is_eval=False, dev_split=0.1):
    ...
    if not is_eval:
        train, dev = shuffle_and_split(new_dial, dev_split=dev_split)
        print(f'train length : {len(train)}')
        print(f'dev length : {len(dev)}')

        with open('./new_train_dials.json', 'w') as f:
            json.dump(train, f, ensure_ascii=False)

        with open('./new_dev_dials.json', 'w') as f:
            json.dump(dev, f, ensure_ascii=False)
    else:
        with open('./new_test_dials.json', 'w') as f:
...
def shuffle_and_split(data: List[Dict], dev_split=0.1):
    num_data = len(data)
...
if __name__=='__main__':
    # make_WOS_like_WOZ('../input/data/train_dataset/train_dials.json', is_eval=False, dev_split=0.1)
